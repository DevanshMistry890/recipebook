...
function RecipeCardSpoon({ recipeName, cookTime, imageUrl, id }) {
  ...
  return (
    <figure className="my-3 my-md-4 tstbite-card">
      <Link to={`/spoon/${id}`} className="tstbite-animation stretched-link rounded-6 d-block">
        <img
          src={currentImageSrc}
          alt={recipeName}
      ...
      <figcaption className="mt-2 text-center">
        <Link to={`/spoon/${id}`} className="text-black d-block mt-1 font-weight-semibold big">
          {recipeName}
        </Link>
...
export default RecipeCardSpoon;